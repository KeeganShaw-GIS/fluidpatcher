...
CHAN = 1
TYPE = "note"  # 'cc' or 'note'
DEC_PATCH = 36  # decrement the patch number
INC_PATCH = 37  # increment the patch number
BANK_INC = 38  # load the next bank

i2c = busio.I2C(board.SCL, board.SDA)
oled = adafruit_ssd1306.SSD1306_I2C(128, 64, i2c)
upperFont = ImageFont.truetype("./DejaVuSerif-Bold.ttf", size=14)
lowerFont = ImageFont.truetype("./DejaVuSerif-Bold.ttf", size=20)
...
def connect_controls():
    ...
    if SELECT_PATCH is not None:
        selectspec = f"0-127=0-{min(len(fp.patches) - 1, 127)}"  # transform CC values into patch numbers
        fp.add_router_rule(
            type="cc", chan=CHAN, par1=SELECT_PATCH, par2=selectspec, patch="select"
        )
...
class HeadlessSynth:
    ...
    def listener(self, sig):
        ...
        if hasattr(sig, "bank") and sig.val > 0:
            banks = sorted(
                [b.relative_to(fp.bankdir) for b in fp.bankdir.rglob("*.yaml")]
            )
            bno = (
                (banks.index(fp.currentbank) + 1) % len(banks)
